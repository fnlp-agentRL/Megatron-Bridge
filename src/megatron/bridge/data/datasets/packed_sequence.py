...
def _pre_pad_data_point(data: dict, max_seq_length: int, max_length_to_pad: int, pad_id: int) -> None:
    ...
    Pads ``input_ids``/``context_ids`` with ``pad_id``, ``loss_mask`` with ``False`` (no loss on pad
    positions), and ``padding_mask`` with ``True`` (artificial pad positions). The chat preprocessing
    ...
    if "input_ids" in data and "padding_mask" not in data:
        ...
        data["padding_mask"] = [False] * len(input_ids)
    ...
    pad_values = {"input_ids": pad_id, "context_ids": pad_id, "loss_mask": False, "padding_mask": True}
